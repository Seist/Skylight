    ...
    public class In
    {
        ...
        public void onMessage(object sender, PlayerIOClient.Message m)
        {
            ...
            switch (Convert.ToString(m.Type))
            {
                ...
                case "add": onAdd(m, w); Console.WriteLine(m);
                ...
                case "b": onB(m, getPlayer(m.GetInt(0)), w);
                ...
                case "left": onLeft(m, getPlayer(m.GetInt(0)), w);
                ...
                case "m": onM(m, getPlayer(m.GetInt(0)), w);
                ...
                case "say": onSay(m, getPlayer(m.GetInt(0)), w);
        ...
        private void onAccess(PlayerIOClient.Message m)
        {
            Out.writeLine("You now have editing rights.", info);
        }

        private void onLostAccess(PlayerIOClient.Message m)
        {
            Out.writeLine("You have lost your editing rights.", info);
        }

        private void onAdd(PlayerIOClient.Message m, World w)
        {
            w.onlinePlayers.Add(new Player { });
        }

        public static event playerEvent _onLeave, _onJoin, _onMovement, _onChat, _onBrick;
        private void onLeft(PlayerIOClient.Message m, Player p, World w)
        {

        }

        private void onInit(PlayerIOClient.Message m, World w)
        {
        ...
        private void onM(PlayerIOClient.Message m, Player p, World w)
        {
