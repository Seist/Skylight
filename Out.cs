...
namespace Skylight
{
    public class Out
    {
        ...
        public void joinRoom(World w)
        {
            // Create a connection, push, and pull.
            // Connection can have some errors, so we add it seperately in a try-catch.

            World temp = new World() { id = w.id, pull = new In() };

            try
            {
                temp.C = client.Multiplayer.JoinRoom(w.id, new Dictionary<string, string>());
            ...
            {
                writeLine("Unable to join room \"" + w.id + "\": " + e.Message, error);
